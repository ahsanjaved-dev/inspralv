/**
 * Optimized Batch Calling Engine for Large-Scale Campaigns
 * 
...
export interface OptimizedBatchConfig {
  campaignId: string
  workspaceId: string
  agentId: string
...
// ============================================================================
// BUSINESS HOURS CHECK (Same as original)
// ============================================================================
...
export function isWithinBusinessHours(
  ...
  try {
    ...
    console.error("[OptimizedBatchCaller] Error checking business hours:", error)
    return true
  }
}
...
async function fetchPendingRecipientsChunk(
  ...
  if (error) {
    console.error("[OptimizedBatchCaller] Error fetching recipients chunk:", error)
    return []
  }
...
async function getPendingRecipientCount(campaignId: string): Promise<number> {
  ...
  if (error) {
    console.error("[OptimizedBatchCaller] Error counting recipients:", error)
    return 0
  }
...
/**
 * Bulk update recipient statuses - OPTIMIZED
 */
async function bulkUpdateRecipientStatuses(
  ...
  if (callingUpdates.length > 0) {
    ...
    if (error) {
      ...
      console.warn("[OptimizedBatchCaller] RPC not available, using individual updates")
...
async function updateCampaignStatsEfficient(
  ...
  if (error) {
    ...
    console.warn("[OptimizedBatchCaller] RPC not available, using manual stats update")
...
async function processChunk(
  ...
  console.log(`[OptimizedBatchCaller] Processing chunk ${chunkIndex + 1} with ${recipients.length} recipients (concurrency: ${concurrency})`)
...
// ============================================================================
// MAIN OPTIMIZED BATCH PROCESSOR
// ============================================================================

/**
 * Run optimized batch calling for a campaign
 * 
 * Key features:
 ...
 * - Resume capability
 */
export async function runOptimizedBatchCaller(
  config: OptimizedBatchConfig,
  ...
  console.log(`[OptimizedBatchCaller] Starting optimized batch for campaign ${config.campaignId}`)
  console.log(`[OptimizedBatchCaller] Config: chunkSize=${chunkSize}, concurrency=${config.concurrencyLimit}, maxTime=${maxProcessingTime}ms`)

  const result: OptimizedBatchResult = {
    success: true,
  ...
  try {
    ...
    console.log(`[OptimizedBatchCaller] Total pending: ${totalPending}, Total chunks: ${result.totalChunks}`)

    if (totalPending === 0) {
      console.log(`[OptimizedBatchCaller] No pending recipients`)
    ...
    while (true) {
      ...
      if (!continueCheck.continue) {
        console.log(`[OptimizedBatchCaller] Stopping: ${continueCheck.reason}`)
      ...
      if (config.businessHoursConfig?.enabled) {
        if (!isWithinBusinessHours(config.businessHoursConfig, config.timezone)) {
          console.log(`[OptimizedBatchCaller] Outside business hours, pausing`)
          result.paused = true
          break
        }
      }
      ...
      if (elapsedTime > maxProcessingTime - 5000) {
        console.log(`[OptimizedBatchCaller] Approaching timeout (${elapsedTime}ms), yielding`)
      ...
      if (recipients.length === 0) {
        console.log(`[OptimizedBatchCaller] No more pending recipients`)
        break
      }
      ...
      console.log(`[OptimizedBatchCaller] Chunk ${chunkIndex + 1} complete: ${chunkResult.successful} success, ${chunkResult.failed} failed (${chunkResult.processingTimeMs}ms)`)

      // Check if all done
      const pendingRemaining = await getPendingRecipientCount(config.campaignId)
      if (pendingRemaining === 0) {
        console.log(`[OptimizedBatchCaller] All recipients processed`)
        break
      }
    ...
    console.log(`[OptimizedBatchCaller] Batch complete: ${result.totalProcessed} processed, ${result.successful} success, ${result.failed} failed (${result.processingTimeMs}ms)`)

    return result

  } catch (error) {
    console.error(`[OptimizedBatchCaller] Fatal error:`, error)
...
/**
 * Start a campaign with optimized batch processing
 * Returns immediately with a batch ID, processing continues in background
 */
export async function startOptimizedCampaign(
  config: OptimizedBatchConfig
): Promise<{ batchId: string; started: boolean; error?: string }> {
  const batchId = `batch-${config.campaignId}-${Date.now()}`
  
  console.log(`[OptimizedBatchCaller] Starting campaign ${config.campaignId}, batchId: ${batchId}`)

  // Fire and forget - but with proper error handling
  runOptimizedBatchCaller(config).then((result) => {
    console.log(`[OptimizedBatchCaller] Campaign ${config.campaignId} batch finished:`, {
      batchId,
    ...
    console.error(`[OptimizedBatchCaller] Campaign ${config.campaignId} batch error:`, error)
  })
